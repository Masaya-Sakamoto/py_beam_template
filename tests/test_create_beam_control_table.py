...
def test_create_fib_rng_beam_control_table():
    ...
    # check
    assert len(beam_control_table) == 196
    assert beam_control_table[0] == {
        'id': 1,
        'dB': 0,
        'theta_d': 0,
...
def test_create_lin_rng_beam_control_table():
    config = arg_parser("tests/conf/lin_rnd/lin_rnd_conf.json")
    beam_template_lst = get_beam_template_from_json(config)
    beam_control_program_lst = get_beam_control_program_from_json(config)
    # beam table作成
    beam_table = []
    ...
    # check
    assert len(beam_control_table) == 157
    assert beam_control_table[0] == {
        'id': 1,
        'dB': 0,
        'theta_d': 0,
